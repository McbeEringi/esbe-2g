...
vec4 water(vec4 col,float weather,highp float time){
	...
	vec4 col2 = col*mix(1.2,1.5,skn);//almost C_REF in ESBE1G
	vec4 col3 = mix(col*1.1,vec4(1.),smoothstep(3.+abs(wPos.y)*.3,0.,abs(wPos.z))*sun*weather*.9);

	vec4 diffuse = mix(col,mix(col2,col3,smoothstep(.5,.9,n)),n);
	return mix(col,diffuse,min(1.,cosT+.5));
}
...
//1が標準,小…暗,大…明
vec3 ambient = mix(mix(mix(/*雨*/vec3(0.8,0.82,1.0),mix(mix(/*夜*/vec3(0.7,0.72,0.8),/*昼*/vec3(1.6,1.6,1.5),daylight.y),/*日没*/vec3(1.7,1.3,1.0),dusk),weather),/*水*/vec3(1.0,1.0,0.9),wf),/*屋内*/vec3(1.2,1.1,1.0),indoor);
...
//ESBE_shadow
float ao = 1.;
if(color.g==color.b)ao = smoothstep(.48*daylight.y,.52*daylight.y,color.g);
