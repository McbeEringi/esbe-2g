...
float4 water(float4 col,float3 p,float3 look,float weather,float sun){
	...
	col.rgb = lerp(col.rgb,FOG_COLOR.rgb,cosT*cosT*sun*.6);
	...
	float4 col2 = col*(lerp(1.4,1.2,skn*sun)-cosT*.1);//almost C_REF in ESBE1G
...
//1が標準,小…暗,大…明
float3 ambient = lerp(lerp(lerp(/*雨*/float3(0.8,0.82,1.0),lerp(lerp(/*夜*/float3(0.7,0.72,0.8),/*昼*/float3(1.57,1.56,1.5),daylight.y),/*日没*/float3(1.7,1.35,0.9),dusk),weather),/*水*/float3(1.,1.,1.),PSInput.wf),/*屋内*/float3(1.2,1.1,1.0),indoor);
...
//ESBEwater
#ifdef FANCY
	if(PSInput.wf+uw > .5)diffuse = water(diffuse,PSInput.cPos,PSInput.wPos,weather,PSInput.uv1.y);
#endif
